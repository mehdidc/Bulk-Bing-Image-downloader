...
def download(query, url,output_dir):
    ...
    for q in query.split():
        ...
        if q in filename:
            ...
            break
    if not ok:
        #print('Skipping {}(keyword:{}) because filename does not contain keyword'.format(filename, query))
        pool_sema.release()
        return
        ...
        try:
            ...
            for index, link in enumerate(links):
                if limit is not None and current + index >= limit:
                    return
                t = threading.Thread(target = download,args = (keyword, link,output_dir))
                t.start()
            last = links[-1]
            current += bingcount
        except IndexError:
